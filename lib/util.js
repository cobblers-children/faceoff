...
import { chartReport, jsonReport } from "bench-node";
...
function chartResults(results) {
  ...
  if (slow.length > 0) {
    ...
    slow.forEach((result) => {
      chartReport(result, { labelWidth: DEFAULT_LABEL_WIDTH, printHeader: false });
      console.log();
    });
...
function toJSON(results) {
  ...
  for (let result of results) {
    ...
    const output = fromConsole(() => jsonReport(input));
